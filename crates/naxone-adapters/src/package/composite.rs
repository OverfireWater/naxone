...
//!   2. NaxOne's own `Extensions/` directory (same format, different root)
//!   3. Extra install paths manually registered in config
//!   4. Legacy `%APPDATA%/NaxOne/Packages/{name}/{version}/` from pre-v2 store
//!
...
use naxone_core::domain::service::{ServiceInstance, ServiceOrigin};
...
impl CompositeScanner {
    ...
    /// * `legacy_packages_root` — the old v1 `%APPDATA%/NaxOne/Packages/`
    ...
    pub fn scan(
        ...
        // --- 2) NaxOne store Extensions (same PHPStudy layout) ---
