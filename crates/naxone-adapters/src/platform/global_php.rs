...
//!
//! 原理：
//! - 在 `%USERPROFILE%\.naxone\bin\` 维护几个 .cmd 文件（php.cmd、php-cgi.cmd、phpize.cmd），
//!   内容形如 `@"<绝对路径>\php.exe" %*`
//! - 切换"全局版本" = 重写这些 .cmd 文件指向新的 PHP 目录
//! - 首次设置时把 `~/.naxone/bin` 追加到用户 PATH（HKCU\Environment\Path）
...
/// 确认 `~/.naxone/bin` 在用户 PATH（HKCU\Environment\Path）里。
...
"#
    ...
    // 写到临时 .ps1
    let temp = std::env::temp_dir().join("naxone-path-fix.ps1");
