...
use naxone_core::error::NaxOneError;
...
fn write_hosts_text(path: &std::path::Path, text: &str) -> Result<(), NaxOneError> {
...
fn map_hosts_error(e: NaxOneError) -> String {
    match e {
        NaxOneError::PermissionDenied(msg) => format!("PERMISSION_DENIED: {}", msg),
        NaxOneError::FileLocked(msg) => format!("FILE_LOCKED: {}", msg),
