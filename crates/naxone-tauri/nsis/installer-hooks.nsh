; NaxOne NSIS installer hooks.
;
; Chooses a default install directory at startup:
;   - D:\NaxOne if the D: drive exists (preferred — keeps C: clean)
;   - C:\NaxOne otherwise
;
...
!macro NSIS_HOOK_PREINSTALL
  ...
  ; ($PROGRAMFILES64\NaxOne or $PROGRAMFILES\NaxOne). If the user
  ; picked something custom we respect it.
  StrCmp $INSTDIR "$PROGRAMFILES64\NaxOne" maybe_redirect 0
  StrCmp $INSTDIR "$PROGRAMFILES\NaxOne" maybe_redirect skip_redirect
...
use_d:
  StrCpy $INSTDIR "D:\NaxOne"
  Goto set_outdir

use_c:
  StrCpy $INSTDIR "C:\NaxOne"
  Goto set_outdir
