...
//! 用途：用户系统里有第三方/独立装的 nginx/mysql/redis 跑着，没在 RustStudy 的安装列表中，
...
use ruststudy_adapters::process::windows::{get_process_exe_path, netstat_snapshot};
...
pub async fn kill_stranger(pid: u32, state: State<'_, AppState>) -> Result<(), String> {
    ...
    if !out.status.success() {
        ...
        let msg = if lower.contains("access is denied") || stderr.contains("拒绝访问") {
            format!("结束 PID {} 失败：权限不足（请以管理员身份运行 RustStudy）", pid)
