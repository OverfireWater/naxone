    ...
    pub fn from_io_with_context(err: std::io::Error, context: &str) -> Self {
        ...
        match err.kind() {
            ErrorKind::PermissionDenied => {
                RustStudyError::PermissionDenied(format!("{}: {}", context, err))
            }
            // Windows 上文件被占用通常是 ErrorKind::Other + os error 32
            _ => {
                let os = err.raw_os_error();
                if os == Some(32) || os == Some(33) {
                    RustStudyError::FileLocked(format!("{}: {}", context, err))
                } else {
                    RustStudyError::Io(err)
                }
            }
        }
    }
...
pub type Result<T> = std::result::Result<T, RustStudyError>;
